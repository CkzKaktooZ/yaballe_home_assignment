...
def test_login_user():
    ...
    response = client.post(
        "/users/login", json={"username": "loginuser", "password": "secret123"}
    )
    assert response.status_code == 200
    data = response.json()
...
def test_login_user_wrong_password():
    response = client.post(
        "/users/login", json={"username": "loginuser", "password": "wrongpassword"}
    )
...
def test_get_my_posts():
    ...
    response = client.get(
        "/users/1/posts", headers={"Authorization": f"Bearer {token}"}
    )
    assert response.status_code == 200
